...
```{r}
"
SELECT pn.product_id, pt.product_name
,   AVG(products_number) AS avg_products_number
FROM (
      SELECT order_id, product_id, COUNT(product_id) 
      OVER(partition by order_id) AS products_number
      FROM order_products__train_tbl
    ) pn, products_tbl pt
WHERE pn.product_id = pt.product_id
GROUP BY pn.product_id, pt.product_name
ORDER BY avg_products_number DESC
LIMIT 10
" %>%
dbGetQuery(sc, .)
